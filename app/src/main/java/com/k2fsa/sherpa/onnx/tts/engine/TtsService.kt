...
class TtsService : TextToSpeechService() {
    ...
    override fun onIsLanguageAvailable(_lang: String?, _country: String?, _variant: String?): Int {
        ...
        if (TtsEngine.isLanguageAvailable(application, lang)) {
    ...
    override fun onGetLanguage(): Array<String> {
        return TtsEngine.defaultLanguage(application)
    }
    ...
    override fun onLoadLanguage(_lang: String?, _country: String?, _variant: String?): Int {
        ...
        return if (TtsEngine.isLanguageAvailable(application, lang)) {
            Log.i(TAG, "creating tts, lang :$lang")
            if (TtsEngine.createTts(application)) {
    ...
    override fun onGetVoices(): MutableList<Voice> {
        return VoiceCatalog.installedVoices(application)
            .map { it.toAndroidVoice() }
            .toMutableList()
    }

    override fun onLoadVoice(voiceName: String?): Int {
        if (voiceName == null) return TextToSpeech.ERROR
        return if (TtsEngine.loadVoice(application, voiceName)) {
    ...
    override fun onGetDefaultVoiceNameFor(
        ...
        return VoiceCatalog.installedVoices(application)
            .firstOrNull { it.locale.isO3Language == lang }
            ?.name
            ?: VoiceCatalog.installedVoices(application).firstOrNull()?.name
    }
    ...
    override fun onSynthesizeText(request: SynthesisRequest?, callback: SynthesisCallback?) {
        ...
        val explicitVoiceName = request.params?.getString(SHERPA_VOICE_PARAM)?.takeIf { it.isNotBlank() }
        val requestedVoiceName = explicitVoiceName ?: request.voiceName
        Log.i(TAG, "text: $text, engineSpeed: $engineSpeed, voiceName: ${request.voiceName}, explicitVoiceName: $explicitVoiceName")
        val voice = TtsEngine.resolveVoice(application, requestedVoiceName, language)
        if (voice == null || !TtsEngine.loadVoice(application, voice.name)) {
            callback.error()
            return
        }
        val selectedSpeakerId = if (requestedVoiceName.isNullOrBlank()) {
            PreferenceHelper(application).getSid()
        } else {
            voice.sid
        }
        TtsEngine.speakerId = selectedSpeakerId
        Log.i(TAG, "synth voice: ${voice.name}, sid: $selectedSpeakerId")
        ...
        tts.generateWithConfigAndCallback(
            text = text,
            config = GenerationConfig(sid = selectedSpeakerId, speed = engineSpeed),
